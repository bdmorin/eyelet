...
class ConversationLoader:
    ...
    def load_all_projects(self, progress_callback=None) -> dict[str, Any]:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_file)):
                ...
                if progress_callback:
                    progress_callback(i + 1, total_files, f"Processed {file_path.name}")
    ...
    def _load_file_data(self, data: dict[str, Any], file_path: Path) -> None:
        ...
        try:
            ...
            conn.execute(
                "DELETE FROM messages WHERE session_id = ?", (conversation.session_id,)
            )
            conn.execute(
    ...
    def get_loading_stats(self) -> dict[str, Any]:
        ...
        if row and row[0]:
            ...
            stats["latest_conversation"] = (
                datetime.fromtimestamp(row[1]) if row[1] else None
            )
