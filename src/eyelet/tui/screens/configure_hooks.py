...
class HookEditorModal(ModalScreen):
    ...
    def __init__(self, hook: Hook | None = None, *args, **kwargs):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="hook-editor"):
            yield Static(
                "⚙️  Edit Hook" if self.hook else "➕ Add Hook", classes="screen-header"
            )
            ...
            current_type = (
                self.hook.type.value if self.hook else HookType.PreToolUse.value
            )
            yield Select(options=hook_types, value=current_type, id="hook-type-select")
            ...
            yield TextArea(
                text=self._get_handler_text(), id="handler-config", language="bash"
            )
    ...
    def on_save(self) -> None:
        ...
        # Create hook
        hook = Hook(
            type=hook_type, handler=handler, matcher=matcher if matcher else None
        )
...
class ConfigureHooksScreen(Screen):
    ...
    def load_hooks(self) -> None:
        ...
        try:
            ...
            self.app.call_from_thread(
                self.update_status, f"Loaded {len(self.hooks)} hooks", "success"
            )
        except Exception as e:
            self.app.call_from_thread(
                self.update_status, f"Failed to load hooks: {e}", "error"
            )
    ...
    def update_table(self) -> None:
        ...
        for i, hook in enumerate(self.hooks):
            ...
            hook_type_str = (
                hook.type.value if hasattr(hook.type, "value") else hook.type
            )
            ...
            handler_type = (
                hook.handler.type.value
                if hasattr(hook.handler.type, "value")
                else hook.handler.type
            )
    ...
    def action_add_hook(self) -> None:
        ...
        def on_dismiss(hook: Hook | None) -> None:
            if hook:
    ...
    def action_edit_hook(self) -> None:
        ...
        if table.cursor_row is not None and 0 <= table.cursor_row < len(self.hooks):
            ...
            def on_dismiss(edited_hook: Hook | None) -> None:
                if edited_hook:
    ...
    def action_test_hook(self) -> None:
        ...
        if table.cursor_row is not None and 0 <= table.cursor_row < len(self.hooks):
            ...
            self.set_timer(
                2.0, lambda: self.update_status("Hook test completed", "success")
            )
        else:
            self.update_status("No hook selected", "warning")
    ...
    def action_save(self) -> None:
        ...
        try:
            ...
            self.app.call_from_thread(
                self.update_status, f"Failed to save: {e}", "error"
            )
