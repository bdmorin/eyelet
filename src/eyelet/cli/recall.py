...
def recall(
    query: str, role: str, tool: str, session: str, since: str, limit: int, no_tui: bool
):
    ...
    try:
        ...
        if stats["total_conversations"] == 0:
            console.print(
                "[yellow]No conversations found in database. Loading from Claude Code logs...[/yellow]"
            )
            ...
            with Progress(
                ...
                def update_progress(current, total, message):
                    if total:
                        progress.update(
                            task, total=total, completed=current, description=message
                        )
            ...
            if load_stats["errors"]:
                console.print(
                    f"[yellow]⚠ {len(load_stats['errors'])} files had errors[/yellow]"
                )
        ...
        if no_tui or not sys.stdout.isatty():
            ...
            class RecallApp(App):
                ...
                def on_mount(self) -> None:
                    ...
                    self.push_screen(
                        RecallScreen(
                            db_path, initial_query=query, initial_filter=search_filter
                        )
                    )
